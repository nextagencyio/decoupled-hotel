...
import { GET_ROOM_BY_PATH } from '@/lib/queries'
import { DrupalRoom } from '@/lib/types'
...
interface RoomByPathData {
  route: {
    entity: DrupalRoom
  } | null
}

async function getRoom(path: string): Promise<DrupalRoom | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<RoomByPathData>({
      query: GET_ROOM_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/rooms/${slug.join('/')}`
  const item = await getRoom(path)

  if (!item) {
    return { title: 'Room Not Found | Hotel' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).pricePerNight && (
                    <div>
                      <dt className="text-sm text-gray-500">Price Per Night</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).pricePerNight}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
