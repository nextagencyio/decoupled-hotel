...
export default function AttractionCard({ item }: AttractionCardProps) {
  return (
    <Link
      href={item.path || '#'}
      className="group bg-slate-900 border border-slate-800 rounded-xl hover:border-accent-500/30 hover:shadow-lg hover:shadow-accent-500/5 transition-all duration-300 overflow-hidden"
    >
      ...
      <div className="p-6">
          {(item as any).distance && (
            <p className="text-sm text-accent-400 font-medium mb-2">{(item as any).distance}</p>
          )}
        ...
        <div className="flex items-center text-accent-400 font-medium text-sm group-hover:gap-2 transition-all">
          Learn more
