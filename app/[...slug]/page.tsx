  ...
  return (
    <div className="text-center py-16">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-12">
        <h1 className="text-2xl font-serif font-light text-white mb-4">Page Not Found</h1>
...
export default async function GenericPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  try {
    ...
    return (
      <div className="min-h-screen bg-slate-950">
        <Header />
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-8 md:pb-12">
          <ErrorBoundary>
            <article className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
              ...
              <div className="p-6 md:p-8">
                <h1 className="font-serif text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light text-white mb-4 md:mb-6">{title}</h1>
                <div className="prose prose-sm sm:prose lg:prose-lg max-w-none prose-invert prose-headings:text-white prose-headings:font-serif prose-headings:font-light prose-p:text-gray-300 prose-a:text-accent-400 prose-strong:text-white prose-blockquote:border-accent-500/30 prose-blockquote:text-gray-400" dangerouslySetInnerHTML={{ __html: bodyHtml }} />
              </div>
            </article>
          </ErrorBoundary>
        </main>
