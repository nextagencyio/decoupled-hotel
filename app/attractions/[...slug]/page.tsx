...
import { GET_ATTRACTION_BY_PATH } from '@/lib/queries'
import { DrupalAttraction } from '@/lib/types'
...
interface AttractionByPathData {
  route: {
    entity: DrupalAttraction
  } | null
}

async function getAttraction(path: string): Promise<DrupalAttraction | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<AttractionByPathData>({
      query: GET_ATTRACTION_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/attractions/${slug.join('/')}`
  const item = await getAttraction(path)

  if (!item) {
    return { title: 'Attraction Not Found | Hotel' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).distance && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).distance}</dd>
                    </div>
                  )}
