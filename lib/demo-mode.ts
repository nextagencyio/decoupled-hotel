...
const mockDataMap: Record<string, any> = {
  ...
  'attractions.json': attractionsData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetRooms') || query.includes('nodeRooms')) {
      return loadMockData('rooms.json')
    }
    ...
    if (query.includes('GetAttractions') || query.includes('nodeAttractions')) {
      return loadMockData('attractions.json')
    }
